...
func main() {
	depths := readInput()
	answerOn1 := NumberTimesTheDepthIncreases(depths)
	fmt.Println("1: ", answerOn1)
	answerOn2 := NumberOfTimesGroupedByThree(depths)
	fmt.Println("2: ", answerOn2)
}
...
func NumberTimesTheDepthIncreases(depths []int) int {
	numberIncreases := 0
...
func GroupByThree(depths []int) []int {
	group := []int{}
	depthsGrouped := []int{}

	for _, depth := range depths {
		group = append(group, depth)

		if len(group) == 3 {
			depthsGrouped = append(depthsGrouped, SumSlice(group))
			group = group[1:]
		}
	}
	return depthsGrouped
}
...
func NumberOfTimesGroupedByThree(depths []int) int {
	groupedDepths := GroupByThree(depths)

	return NumberTimesTheDepthIncreases(groupedDepths)
}
