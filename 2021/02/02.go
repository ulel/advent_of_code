...
func parseCommands(commandsReader io.Reader) []command {

	scanner := bufio.NewScanner(commandsReader)
	...
	for scanner.Scan() {
		...
		units, err := strconv.Atoi(splitLine[1])

		if err != nil {
			log.Fatal(err)
		}

		commands = append(commands, command{direction: splitLine[0], units: units})
	}
...
func main() {
	...
	fmt.Println("1: ", horizontal*depth)

	_, depthWithAim := getFinalPositionWithAim(commands)
	fmt.Println("1: ", horizontal*depthWithAim)
}

func getFinalPositionWithAim(commands []command) (int, int) {
	horizontal := 0
	depth := 0
	aim := 0
