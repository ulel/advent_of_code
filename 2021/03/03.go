...
func getGammaRate(report []string) (uint, int) {
	sizeOfNumber := len(report[0])
	lengthOfReport := len(report)
	numberOfOnes := make([]uint, sizeOfNumber)

	for _, number := range report {
		for i, value := range number {
			if value == '1' {
				numberOfOnes[sizeOfNumber-i-1] += 1
			}
		}
	}

	return convertToGammaRate(numberOfOnes, lengthOfReport), sizeOfNumber
}

func convertToGammaRate(input []uint, lengthOfReport int) uint {
	total := uint(0)

	for i, value := range input {
		if value > uint(lengthOfReport/2) {
			total = total | 1<<i
		}
	}
...
func main() {
	report := readInput()
	gammaRate, lengthOfNumber := getGammaRate(report)
	epsilonRate := getEpsilonRateFromGammaRate(gammaRate, lengthOfNumber)
	fmt.Println("1: ", gammaRate*epsilonRate)

	oxygenGeneratorRating := getOxygenGeneratorRating(report)
	co2ScrubberRating := getCo2ScrubberRating(report)

	fmt.Println("2: ", oxygenGeneratorRating*co2ScrubberRating)
}

func getEpsilonRateFromGammaRate(gammaRate uint, sizeOfNumber int) uint {
	return ((1 << sizeOfNumber) - 1) - gammaRate
}

func getOxygenGeneratorRating(report []string) uint {
	sizeOfNumber := len(report[0])
	...
	oxygenGeneratorRating, err := strconv.ParseInt(report[0], 2, 32)

	if err != nil {
		panic(err)
	}

	return uint(oxygenGeneratorRating)
}

func getCo2ScrubberRating(report []string) uint {
	sizeOfNumber := len(report[0])
	...
	co2ScrubberRating, err := strconv.ParseInt(report[0], 2, 32)

	if err != nil {
		panic(err)
	}

	return uint(co2ScrubberRating)
}
