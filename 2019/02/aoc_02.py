...
def run_program(program):
    ...
    while (opcode := program[instruction_pointer]) != 99:
        operator = get_operator(opcode)
        ...
        program[destination] = operator(program[parameter_1], program[parameter_2])
        instruction_pointer += 4

    return program


def find_noun_and_verb(memory, value_to_find):
    for noun in range(0, 100):
        for verb in range(0, 100):
            next_memory = copy.deepcopy(memory)
            next_memory[1] = noun
            next_memory[2] = verb
            if run_program(next_memory)[0] == value_to_find:
                return (noun, verb)
    return (None, None)


def main():
    instructions = restore_program(aoc_02_input.get_instructions())
    print(f"part_1: {run_program(instructions)[0]}")

    instructions = aoc_02_input.get_instructions()
    (noun, verb) = find_noun_and_verb(instructions, 19690720)
    print(f"part_2: {noun*100+verb}")
