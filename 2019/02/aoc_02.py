...
def run_program(program):
    ...
    while (op_code := program[current_pos]) != 99:
        operator = get_operator(op_code)

        source_1 = program[current_pos + 1]
        source_2 = program[current_pos + 2]
        destination = program[current_pos + 3]

        program[destination] = operator(program[source_1], program[source_2])
        current_pos += 4
