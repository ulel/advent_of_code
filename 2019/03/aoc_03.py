...
def trace_cable(
    ...
    current_tile = (0, 0)
    tiles[current_tile] = cable_id if current_tile not in tiles else -1

    for next_move in path:
        for tile in _get_tiles_in_path(current_tile, next_move):
            if tile in tiles and tiles[tile] != cable_id:
                tiles[tile] = -1
...
def main():
    ...
    for cable in aoc_03_input.get_cable_instructions():
        (closest_cross, _) = trace_cable(cable_id, cable, tiles)
        cable_id += 1
