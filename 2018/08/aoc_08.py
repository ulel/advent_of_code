...
def calculate_sum(data):
    ...
    for _ in range(number_of_childs):
        sum_of_childs, rest = calculate_sum(rest)
        sum_of_metadata += sum_of_childs

    sum_of_metadata += sum(rest[:metadata_length])

    return (sum_of_metadata, rest[metadata_length:])


def main():
    (part_1, _) = calculate_sum(aoc_08_input.get_input())

    print(f"Advent of Code day 08 part1: {part_1}")
